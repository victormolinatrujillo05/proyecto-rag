...
## 2. Conceptos Clave y Fundamentos TeóricosS
...
* **LlamaIndex**: Es el orquestador que conecta los documentos con el modelo. Se encarga de la fragmentación (chunking), la creación del índice y la gestión de la memoria.
* **ChromaDB**: Base de datos vectorial persistente que almacena los embeddings. Su papel es crucial para no tener que volver a procesar los documentos en cada inicio.
* **Streamlit**: Proporciona una interfaz web intuitiva para que cualquier usuario interactúe con el sistema de forma visual.
...
Inicialmente, el sistema recreaba el índice en cada ejecución, lo que consumía excesivo tiempo de CPU. Se decidió implementar **StorageContext** en LlamaIndex para forzar la persistencia en el disco local dentro de la carpeta `/chroma_db`. Esto optimizó el arranque del sistema notablemente.

### 4.2. Limitaciones de Hardware y Soluciones
Durante las pruebas en el entorno local, se detectaron bloqueos cuando el sistema intentaba procesar documentos extensos o cuando la RAM disponible era escasa. Para solucionar esto:
1. Se ajustó el tamaño de los fragmentos de texto (**chunk_size**) para que el modelo pudiera procesarlos más rápido.
2. Se incrementó el **timeout** de las peticiones a Ollama para evitar que la interfaz de Streamlit diera error mientras el modelo generaba la respuesta.

---

## 5. Conclusión
La combinación de LlamaIndex y Ollama ofrece una flexibilidad total para adaptar la IA a cualquier base de conocimientos específica sin los costes asociados a las APIs comerciales, garantizando seguridad y soberanía sobre los datos procesados.